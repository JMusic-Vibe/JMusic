...
if command -v jq >/dev/null 2>&1; then
    ...
    if [ -z "$DOWNLOAD_URL" ] || [ "$DOWNLOAD_URL" = "null" ]; then
        # Fallback: any frontend dist tar.gz (non-lite), otherwise any .tar.gz
        DOWNLOAD_URL=$(echo "$RELEASE_INFO" | jq -r '.assets[] | select(.browser_download_url | test("frontend.*\\.tar\\.gz$") and (test("lite") | not)) | .browser_download_url' | head -1)
    fi
    if [ -z "$DOWNLOAD_URL" ] || [ "$DOWNLOAD_URL" = "null" ]; then
        DOWNLOAD_URL=$(echo "$RELEASE_INFO" | jq -r '.assets[] | select(.browser_download_url | test("\\.tar\\.gz$")) | .browser_download_url' | head -1)
    fi
else
    ...
    if [ -z "$DOWNLOAD_URL" ]; then
        DOWNLOAD_URL=$(echo "$RELEASE_INFO" | grep -o '"browser_download_url":"[^"]*frontend[^"]*\.tar\.gz"' | grep -v 'lite' | head -1 | sed 's/.*"browser_download_url":"\([^"]*\)".*/\1/')
    fi
    if [ -z "$DOWNLOAD_URL" ]; then
