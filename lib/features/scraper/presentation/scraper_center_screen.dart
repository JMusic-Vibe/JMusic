...
class _ScraperCenterScreenState extends ConsumerState<ScraperCenterScreen> {
  ...
  void _showSelectionActionsSheet(BuildContext context) {
    ...
    BottomActionSheet.show(
      ...
      actions: [
        ...
        ActionItem(
          icon: Icons.person,
          title: l10n.scrapeArtistAvatars,
          onTap: _scrapeSelectedArtists,
        ),
        ActionItem(
          icon: Icons.edit,
          title: l10n.editTags,
  ...
  Future<void> _scrapeSelectedArtists() async {
    ...
    final ok = await ref.read(artistScraperControllerProvider).scrapeArtistsForSongs(songs);
    if (mounted) {
      ...
      child: Builder(
        ...
        appBar: AppBar(
          ...
          title: _showSearch
              ...
              : Text(_isSelectionMode ? l10n.selectedCount(_selectedIds.length) : l10n.scraperCenter),
              ...
              : [
                  if (_currentTab() == _ScraperTab.artists)
                    IconButton(
                      ...
                      tooltip: l10n.batchScrapeArtists,
                      onPressed: () async {
                        final songs = songsAsync.value ?? [];
                        final artists = _buildArtistItems(songs);
                        final names = artists.map((e) => e.name).toList();
                        final count = await ref.read(artistScraperControllerProvider).scrapeArtistsByNames(names);
                        if (mounted) {
                          CapsuleToast.show(context, l10n.scrapeArtistAvatarsResult(count));
                        }
                      },
                    ),
                  if (!_showSearch)
            ...
            child: songsAsync.when(
              data: (songs) {
                ...
                return TabBar(
                  isScrollable: true,
                  tabs: [
                    Tab(text: '${l10n.scraperCategoryUnscraped} (${counts[_ScraperTab.unscraped]})'),
                    Tab(text: '${l10n.scraperCategoryScraped} (${counts[_ScraperTab.scraped]})'),
                    Tab(text: '${l10n.scraperCategoryHasLyrics} (${counts[_ScraperTab.hasLyrics]})'),
                    Tab(text: '${l10n.scraperCategoryNoLyrics} (${counts[_ScraperTab.noLyrics]})'),
                    Tab(text: '${l10n.scraperCategoryMissingCover} (${counts[_ScraperTab.missingCover]})'),
                    Tab(text: '${l10n.scraperCategoryMissingInfo} (${counts[_ScraperTab.missingInfo]})'),
                    Tab(text: '${l10n.scraperCategoryArtists} (${counts[_ScraperTab.artists]})'),
                  ],
                );
              },
        ...
        body: songsAsync.when(
          data: (songs) {
            return TabBarView(
              children: _tabs.map((tab) => _buildListForTab(context, songs, tab, prefs)).toList(),
            );
          },
  ...
  Map<_ScraperTab, int> _buildCounts(List<Song> songs, PreferencesService prefs) {
    return {
      _ScraperTab.unscraped: _filterByIssue(songs, _ScraperTab.unscraped, prefs).length,
      _ScraperTab.scraped: _filterByIssue(songs, _ScraperTab.scraped, prefs).length,
      _ScraperTab.hasLyrics: _filterByIssue(songs, _ScraperTab.hasLyrics, prefs).length,
      _ScraperTab.noLyrics: _filterByIssue(songs, _ScraperTab.noLyrics, prefs).length,
      _ScraperTab.missingCover: _filterByIssue(songs, _ScraperTab.missingCover, prefs).length,
      _ScraperTab.missingInfo: _filterByIssue(songs, _ScraperTab.missingInfo, prefs).length,
      _ScraperTab.artists: _buildArtistItems(songs).length,
    };
  }

  Widget _buildListForTab(BuildContext context, List<Song> songs, _ScraperTab tab, PreferencesService prefs) {
    final l10n = AppLocalizations.of(context)!;
          ...
          return ListTile(
            leading: ClipOval(
              child: SizedBox(
                width: 44,
                height: 44,
                child: CoverArt(path: avatar, fit: BoxFit.cover, isVideo: false),
              ),
            ),
            title: Text(item.name, style: Theme.of(context).textTheme.titleMedium?.copyWith(color: Theme.of(context).colorScheme.onSurface)),
            subtitle: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(l10n.songCount(item.count), style: Theme.of(context).textTheme.bodySmall?.copyWith(color: Theme.of(context).colorScheme.onSurfaceVariant)),
                if (sourceLabel != null)
                  Text(
                ...
                BottomActionSheet.show(
                  ...
                  actions: [
                    ...
                    ActionItem(
                      ...
                      title: l10n.scrapeArtistAvatars,
                      onTap: () async {
                        final ok = await ref.read(artistScraperControllerProvider).scrapeArtist(item.name, force: true);
                        if (mounted) {
                          CapsuleToast.show(context, l10n.scrapeArtistAvatarsResult(ok ? 1 : 0));
                        }
                      },
                    ),
                  ],
    ...
    if (filteredSongs.isEmpty) {
      ...
      final emptyText = tab == _ScraperTab.unscraped ? l10n.allSongsScraped : l10n.noMatchingSongs;
      return Center(
        child: Text(
          emptyText,
        ...
        return ListTile(
          ...
          trailing: _isSelectionMode
              ...
              : IconButton(
                  icon: const Icon(Icons.more_vert),
                  tooltip: l10n.more,
                  onPressed: () => _showItemActions(context, song),
                ),
        );
      },
    );
  ...
  List<_ArtistItem> _buildArtistItems(List<Song> songs) {
    ...
    final list = map.entries.map((e) => _ArtistItem(name: e.key, count: e.value)).toList();
    list.sort((a, b) => a.name.compareTo(b.name));
    return list;
  }

  void _showItemActions(BuildContext context, Song song) {
    final l10n = AppLocalizations.of(context)!;
    BottomActionSheet.show(
      context,
      title: song.title,
      actions: [
        ...
        ActionItem(
          ...
          title: l10n.scrapeArtistAvatars,
          onTap: () async {
            await ref.read(artistScraperControllerProvider).scrapeArtistsForSongs([song]);
            if (mounted) {
  ...
  List<Song> _filterSongs(List<Song> songs, _ScraperTab tab, PreferencesService prefs) {
  ...
  bool _missingCover(Song song) => song.coverPath == null || song.coverPath!.trim().isEmpty;

  bool _missingLyrics(Song song) => song.lyrics == null || song.lyrics!.trim().isEmpty;

  bool _hasLyrics(Song song) => !_missingLyrics(song);

  bool _missingInfo(Song song) => !_hasKnownArtist(song) || _isUnknownOrEmpty(song.album);

  List<Song> _filterByIssue(List<Song> songs, _ScraperTab tab, PreferencesService prefs) {
    switch (tab) {
      ...
      case _ScraperTab.missingCover:
        return songs.where(_missingCover).toList();
      case _ScraperTab.missingInfo:
      default:
        return songs.where(_missingInfo).toList();
    }
  }
}

enum _ScraperTab { unscraped, scraped, hasLyrics, noLyrics, missingCover, missingInfo, artists }
