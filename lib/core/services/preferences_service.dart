...
class PreferencesService {
  ...
  static const String keyOpenListProxyMode = 'openlist_proxy_mode'; // 'system', 'custom', 'none'
  ...
  static const String keyScraperSourceMusicBrainz = 'scraper_source_musicbrainz';
  ...
  static const String keyLyricsDisplayMode = 'lyrics_display_mode'; // 'off', 'compact', 'full'
  ...
  String get openListProxyMode => _prefs.getString(keyOpenListProxyMode) ?? 'none';
  String get openListProxyHost => _prefs.getString(keyOpenListProxyHost) ?? '127.0.0.1';
  ...
  bool get scraperSourceMusicBrainz => _prefs.getBool(keyScraperSourceMusicBrainz) ?? true;
  bool get scraperSourceItunes => _prefs.getBool(keyScraperSourceItunes) ?? true;
  ...
  String get lyricsDisplayMode => _prefs.getString(keyLyricsDisplayMode) ?? 'off';
  ...
  Future<void> setOpenListPort(int port) async => await _prefs.setInt(keyOpenListPort, port);
  Future<void> setOpenListProxyMode(String mode) async => await _prefs.setString(keyOpenListProxyMode, mode);
  Future<void> setOpenListProxyHost(String host) async => await _prefs.setString(keyOpenListProxyHost, host);
  Future<void> setOpenListProxyPort(int port) async => await _prefs.setInt(keyOpenListProxyPort, port);
  Future<void> setOpenListAutoStart(bool autoStart) async => await _prefs.setBool(keyOpenListAutoStart, autoStart);
  ...
  Future<void> setScraperSourceMusicBrainz(bool v) async => await _prefs.setBool(keyScraperSourceMusicBrainz, v);
  Future<void> setScraperSourceItunes(bool v) async => await _prefs.setBool(keyScraperSourceItunes, v);
  ...
  Future<void> setLyricsDisplayMode(String mode) async => await _prefs.setString(keyLyricsDisplayMode, mode);
  Future<void> setShuffleEnabled(bool enabled) async => await _prefs.setBool(keyShuffleEnabled, enabled);
  Future<void> setLoopMode(String mode) async => await _prefs.setString(keyLoopMode, mode);
  Future<void> setDesktopVolume(double volume) async => await _prefs.setDouble(keyDesktopVolume, volume);
